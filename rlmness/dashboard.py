...
# Thin borders with the panel name set into the top rule, and a palette held
# well back: the code and the output are the only things on screen worth
# looking at directly, so nothing else competes for attention.
STYLE = """
...
class RunPanes:
    """The tree, the namespace and the step panes over one `RunTree`.

    Shared by the live dashboard and by the view of a run read back from its
    trace, so a run looks the same whether it is happening or finished.
    """

    def _init_panes(self, tree: RunTree) -> None:
        self.state = tree
    ...
    def _compose_panes(self) -> ComposeResult:
